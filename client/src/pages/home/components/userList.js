...
function UsersList({searchKey, socket, onlineUser}){
    ...
    const { allUsers, allChats, user: currentUser, selectedChat } = useSelector(state => state.userReducer);
    // user: currentUser , currentUser is alias for user as user name is alredy used
    ...
    const openChat = (selectedUserId) => {
        ...
        // console.log("Opening chat with", selectedUserId);
    ...
    const getUnreadMessageCount = (userId) => {
        ...
        //  const isSelected = IsSelectedChat(currentUser); 
    ...
    //             <div className="selected-user filtered-user">
    ...
    getData().map(obj => {
            ...
            if (!user || !user._id) return null; // <== added safety
                ...
                <div className={ IsSelectedChat(user) ? "selected-user" : "filtered-user" }>
                    <div className="filter-user-display">
                        {user.profilePic && <img src={user.profilePic} 
                                                alt="Profile Pic" 
                                                className="user-profile-image" 
                                                style={onlineUser.includes(user._id) ? {border: '#04f340 3px solid'} : {}} 
                                            />}
